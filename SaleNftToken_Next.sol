...
contract SaleNftToken {
    ...
    mapping(uint256 => bool) private isContainsOrderIdsOfAddress; //OrderIdsOfAddress에 orderId가 있는가?
    ...
    function addOnSaleOrderIdsOfAddress(uint256 _orderId, address _onwer) private {
        isContainsOrderIdsOfAddress[_orderId] = true;
        onSaleOrderIdsOfAddress[_onwer].push(_orderId);
    }

    function getIsContainsOrderIdsOfAddress(uint256 _orderId) view private returns (bool) {
        return isContainsOrderIdsOfAddress[_orderId];
    }

    function removeOnSaleOrderIdsOfAddress(uint256 _orderId, address _onwer) private {
        delete isContainsOrderIdsOfAddress[_orderId];
